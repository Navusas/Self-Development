...
public class Client extends Player {
    ...
    public void clearQInterface() {
        ...
        getPA().sendFrame126("Demonium: " + PlayerHandler.getPlayerCount() + " Online", 29155); //Tab Title

        getPA().sendFrame126("Owner", 29161); //1st section title
        getPA().sendFrame126("@cya@Navus", 29162); //1rd section content
    ...
    public boolean isOwner() {
        return playerName.equalsIgnoreCase("mouxime");
    }
    ...
    public void sendAchieved99_Message(String s) {
        synchronized (this) {
            if (getOutStream() != null) {
                outStream.createFrameVarSize(253);
                outStream.writeString("<shad=" + SubstringColors.RED + ">[System]" + s + "</shad>");
                outStream.endFrameVarSize();
            }
        }
    }
    ...
    public void sendSystemMessage(String s) {
        synchronized (this) {
            if (getOutStream() != null) {
                outStream.createFrameVarSize(253);
                outStream.writeString("<col=" + SubstringColors.RED + ">[System]</col>" + s);
                outStream.endFrameVarSize();
            }
        }
    }
    ...
    public void sendMessage(String s, String color) {
        synchronized (this) {
            if (getOutStream() != null) {
                outStream.createFrameVarSize(253);
                outStream.writeString("<col=" + color + ">" + s + "</col>");
                outStream.endFrameVarSize();
            }
        }
    }
    ...
    public void sendDelayedMessage(String s, int secsUntilDisplay) {
        ...
        // start the thread which implements Runnable
        new Thread(() -> {
            while (System.currentTimeMillis() < currentTime + (secsUntilDisplay * 1000)) ;
            sendMessage(s);
        }).start();
    }
    ...
    public boolean checkVotes(String playerName) {
        try {
            ...
            if (results.length() > 0) {
                return results.equals("user needs reward...");
            }
...
/*if(totalstored > 0) {
    ...
    public boolean wearingArmor() {
        if (playerEquipment[playerHat] > 0)
            ...
            return true;
        else return playerEquipment[playerRing] > 0;
    }
    ...
    public void process() {
        ...
        getPA().sendFrame126("Demonium: " + PlayerHandler.getPlayerCount() + " Online", 29155);
        ...
        getPA().sendFrame126("@lre@Leveling Points: @cya@" + levelingPoints + " ", 29169);
        getPA().sendFrame126("@lre@Donator Credits: @cya@" + Credits + "  ", 29170);
        getPA().sendFrame126("@lre@Vote Points: @cya@" + votingPoints + "  ", 29171);
        getPA().sendFrame126("@lre@Pk Points: @cya@" + pkPoints + "  ", 29172);
        getPA().sendFrame126("@lre@Dung Points: @cya@" + dungPoints + " ", 29173);
        getPA().sendFrame126("@lre@Player Kills: @cya@" + KC + " ", 29174);
        getPA().sendFrame126("@lre@Player Deaths: @cya@" + DC + " ", 29175);
        getPA().sendFrame126("@lre@Exp Lock Status: @cya@" + xpLock + " ", 29176);
        getPA().sendFrame126("@lre@Slayer Task: @cya@" + Server.npcHandler.getNpcListName(slayerTask) + " ", 29177);
        getPA().sendFrame126("@lre@Amount: @cya@" + taskAmount + " ", 29178);
        getPA().sendFrame126("", 29179);
        getPA().sendFrame126("@lre@Staff of Demonium", 29266);
...
//            }
    ...
    public synchronized boolean processQueuedPackets() {
        ...
        timeOutCounter = 0;
        return processPackets <= Config.MAX_PROCESS_PACKETS;
    }
