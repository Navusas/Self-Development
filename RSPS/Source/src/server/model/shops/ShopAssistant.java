...
//voting store
    ...
    public boolean buyItem(int itemID, int fromSlot, int amount) {
        ...
        if (amount > 0) {
            ...
            if (c.myShopId == 48 || c.myShopId == 47 || c.myShopId == 73 || c.myShopId == 74) {  // PK SHOP
                handleOtherShop(itemID, amount, fromSlot);
                return false;
            }
            if (c.myShopId == 84 || c.myShopId == 85) {     // DUNGEONEERING
                handleOtherShop(itemID, amount, fromSlot);
                return false;
            }

            totalPrice = (int) Math.floor(getItemShopValue(itemID, 0, fromSlot));
            handleTokkulAndDonateGold_Buying(amount,totalPrice,itemID,fromSlot);
            Misc.println("Total Price : " + totalPrice);
            for (int i = 0; i < amount; i++) {
                ...
                if (Slot == -1) {
                    ...
                    if (c.playerItemsN[Slot] >= totalPrice) {
                        if (c.getItems().freeSlots() > 0) {
                            buyDelay = System.currentTimeMillis();
                            c.getItems().deleteItem(995, c.getItems().getItemSlot(995), totalPrice);
    ...
    public void handleTokkulAndDonateGold_Buying(int amount, int totalPrice, int itemID, int fromSlot) {
        int Slot1 = 0;//Tokkul
        ...
        for (int i = amount; i > 0; i--) {
            ...
            if (c.myShopId == 29 || c.myShopId == 30 || c.myShopId == 31) {
                if (c.playerItemsN[Slot1] >= totalPrice) {
                    if (c.getItems().freeSlots() > 0) {
                        buyDelay = System.currentTimeMillis();
                        c.getItems().deleteItem(6529, c.getItems().getItemSlot(6529), totalPrice);
                        c.getItems().addItem(itemID, 1);
                        ShopHandler.ShopItemsN[c.myShopId][fromSlot] -= 1;
                        ShopHandler.ShopItemsDelay[c.myShopId][fromSlot] = 0;
                        if ((fromSlot + 1) > ShopHandler.ShopItemsStandard[c.myShopId]) {
                ...
                if (c.playerItemsN[Slot3] >= totalPrice) {
                    if (c.getItems().freeSlots() > 0) {
                        buyDelay = System.currentTimeMillis();
                        c.getItems().deleteItem(5555, c.getItems().getItemSlot(5555), totalPrice);
                        c.getItems().addItem(itemID, 1);
                        ShopHandler.ShopItemsN[c.myShopId][fromSlot] -= 1;
                        ShopHandler.ShopItemsDelay[c.myShopId][fromSlot] = 0;
                        if ((fromSlot + 1) > ShopHandler.ShopItemsStandard[c.myShopId]) {
    ...
    public void handleOtherShop(int itemID, int amount, int fromSlot) {
        boolean purchased = false;
        for (int i = 0; i < amount; i++) {
            purchased = false;
            if (c.getItems().freeSlots() <= 0) {
                c.sendSystemMessage("You don't have enough space in your inventory");
                break;
            } else if (ShopHandler.ShopItemsN[c.myShopId][fromSlot] <= 0) {
                c.sendSystemMessage("Item is currently unavailable. Please try again later");
                break;
            }
            if (c.myShopId == 17) {
            ...
            } else if (c.myShopId == 73 || c.myShopId == 74) {                 // PK POINT SHOP
            ...
            } else if (c.myShopId == 48 || c.myShopId == 47) {                // PK POINT STORE
            ...
            } else if (c.myShopId == 84 || c.myShopId == 85) { // DUNGEONEERING
            ...
            resetShop(c.myShopId);                  // update shop frame

            //        else if (c.myShopId == 91) {
//            if (c.levelingPoints >= getSpecialItemValue(itemID)) {
//                if (c.getItems().freeSlots() > 0) {
//                    c.levelingPoints -= getSpecialItemValue(itemID);
//                    c.getItems().addItem(itemID, 1);
//                    c.getItems().resetItems(3823);
//                }
//            } else {
