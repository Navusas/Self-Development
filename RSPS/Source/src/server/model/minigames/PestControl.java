...
public class PestControl {
		...
		for (int j = 0; j < Server.playerHandler.players.length; j++) {
			if (Server.playerHandler.players[j] != null) {
				if (Server.playerHandler.players[j].inPcGame()) {
					...
					if (won && c.pcDamage > 4) {
						c.sendMessage("You have won the pest control game and have been awarded 15 pest control points.");
						c.pcPoints += 15;
	...
	public void movePlayer(int index) {
		...
		if (c.combatLevel < 75) {
			c.sendMessage("You must be at least 75 combat level to enter this boat.");
			return;
		}
